...
bool initialInfo(string & clientsFile, string & transactionsFile, string & ordersFile, string &companysFile) {
	...
	return (validFile(clientsFile) && validFile(transactionsFile) && validFile(ordersFile) 
		&&validFile(companysFile) && !cin.fail());
}
...
unsigned short int companyOptions() {
	...
	cout << TAB << "1 - List ALL companys" << endl;
	cout << TAB << "2 - Insert a new company" << endl;
	cout << TAB << "3 - Remove an existing company" << endl;
	cout << TAB << "4 - Change maximum transaction value of one company" << endl;
	cout << TAB << "5 - Exit sub-menu" << endl << endl;
	string msg = TAB; msg += "Your option: ";
	option = getUnsignedShortInt(1, 5, msg);
